...
const {GOOGLE_WALLET_WEBSERVICE_PATH} = process.env
const VALID_TOKEN1_HASH = '14e21852abe0b7609b6659ffed526ab9d261c8a43c72481043a6141f782e3df0'
test.describe('googleWallet', async () => {   
  
  test('Return PASS_GENERATION_ERROR after success because no default renderer is registered', async () => {        
    const result = await fetch(`http://localhost:4010${GOOGLE_WALLET_WEBSERVICE_PATH}/download/test-token-1?hash=${VALID_TOKEN1_HASH}`);    
  ...
  test('Return HASH_MISMATCH Error when invalid hash is provided', async () => {        
    const result = await fetch(`http://localhost:4010${GOOGLE_WALLET_WEBSERVICE_PATH}/download/test-token-1?hash=14e21852abe0b7609b6659ffed526ab9d261c8a43c72481043a6141f782e3`);    
  ...
  test('Return Not found / 4040 when non existing token ID is provided', async () => {        
    const result = await fetch(`http://localhost:4010${GOOGLE_WALLET_WEBSERVICE_PATH}/download/invalid-id?hash=${VALID_TOKEN1_HASH}`);    
