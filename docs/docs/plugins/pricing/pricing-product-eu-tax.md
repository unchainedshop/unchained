...
Rates for all 27 member states are bundled in [`eu-tax-rates.json`](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/tax/eu-tax-rates.json) as era lists (`{ validFrom, rate }`), verified against the [European Commission's VAT database](https://ec.europa.eu/taxation_customs/tedb/#/vat-search). The applicable rate follows the order date, so historical recalculations stay correct (including temporary cuts such as Germany's 16% in H2 2020). Rates are never fetched at runtime — updates land as reviewed diffs via the repository's `update-tax-rates` skill.
...
| Version | `1.0.0` |
| Order Index | `80` |
| Source | [pricing/product-eu-tax.ts](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/product-eu-tax.ts) |
