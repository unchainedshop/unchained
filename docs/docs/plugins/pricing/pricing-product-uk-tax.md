...
Rates are bundled in [`uk-tax-rates.json`](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/tax/uk-tax-rates.json) as era lists (`{ validFrom, rate }`), verified against [gov.uk](https://www.gov.uk/vat-rates) and the House of Commons Library rate history. The applicable rate follows the order date; updates land as reviewed diffs via the repository's `update-tax-rates` skill.
...
| Version | `1.0.0` |
| Order Index | `80` |
| Source | [pricing/product-uk-tax.ts](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/product-uk-tax.ts) |
