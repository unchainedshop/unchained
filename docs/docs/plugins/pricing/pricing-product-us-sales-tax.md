...
Statewide base rates are bundled in [`us-tax-rates.json`](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/tax/us-tax-rates.json) as era lists (`{ validFrom, rate }`), cross-checked against the state Departments of Revenue and the Tax Foundation's state rate table. Recent changes are era-modeled (e.g. Louisiana 4.45% → 5% on 2025-01-01, DC's enacted increase to 7% on 2026-10-01). The NOMAD states (NH, OR, MT, AK, DE) carry 0%. Updates land as reviewed diffs via the repository's `update-tax-rates` skill.
...
| Version | `1.0.0` |
| Order Index | `80` |
| Source | [pricing/product-us-sales-tax.ts](https://github.com/unchainedshop/unchained/blob/v4.8.x/packages/plugins/src/pricing/product-us-sales-tax.ts) |
