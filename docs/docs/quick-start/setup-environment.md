...
Unchained Engine requires Node.js 26 or newer. For development in the engine monorepo, use the version specified in `.nvmrc`.
...
**Alternative: PostgreSQL via FerretDB 2.x**
1. Set up PostgreSQL
2. Start the FerretDB 2.x Docker container (1.x is not supported — text indexes are required)
