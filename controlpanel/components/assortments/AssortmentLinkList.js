...
export default compose(
  ...
  `),
  graphql(gql`
    mutation addAssortmentLink($media: Upload!, $assortmentId: ID!) {
      addAssortmentLink(media: $media, assortmentId: $assortmentId) {
        _id
        tags
      }
    }
  ...
  withHandlers({
    ...
    onDrop: ({ assortmentId, addAssortmentLink }) => async (files) => {
      const media = files[0];
