...
export default compose(
  ...
  withFormSchema(({ data: { countries } }) => ({
    ...
    'pricing.$.countryCurrency': {
      ...
      allowedValues: (countries || [])
        .reduce((allCombinations, country) => [
          ...allCombinations,
          `${country.isoCode} / ${country.defaultCurrency && country.defaultCurrency.isoCode}`,
        ], [null]),
    },
    'pricing.$.countryCode': {
      optional: true,
  ...
  withFormModel(({ data: { countries, productCatalogPrices } = {} }) => {
    ...
    productPricing.forEach(({
      ...
      const suffix = (maxQuantity && maxQuantity > 0)
        ? `${maxQuantity}`
        : '';
    ...
    const pricing = Object
      .keys(productPricingMap)
      .map(key => productPricingMap[key] && ({
