...
const indexSnippet = [
  ...
  [
    ...
    'If docs missing, run: npx --package @unchainedshop/platform unchained download-llm-docs',
    ...[...markdownsByDir]
...
const legacyAgentsMdPath = join(process.cwd(), 'agents.md');
const claudeExists = existsSync(claudeMdPath);
const agentsExists = existsSync(agentsMdPath);
const legacyAgentsExists = !agentsExists && existsSync(legacyAgentsMdPath);

if (claudeExists) upsertIndex(claudeMdPath);
if (agentsExists) upsertIndex(agentsMdPath);
if (legacyAgentsExists) upsertIndex(legacyAgentsMdPath);

if (!claudeExists && !agentsExists && !legacyAgentsExists) {
