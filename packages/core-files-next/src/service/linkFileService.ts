...
export const linkFileService: LinkFileService = async ({ fileId, size, type, url }, requestContext) => {
  const { modules: { files }, userId } = requestContext;
  const file = await files.findFile({ fileId });
  if (!file) throw new Error(`Media with id ${fileId} Not found`);
  await files.update(file._id, { size, type, expires: null, url }, userId);
