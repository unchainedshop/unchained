 ...
 * User input is treated as literal text apart from wildcard expansion.
 */
const escapeRegexForDebug = (pattern: string): string => {
  // Escape all regex special characters except * (which we convert to .*)
  // Special chars: . + ? ^ $ { } ( ) | [ ] \
  // Hyphens are literal outside character classes and need no escaping.
  return pattern
 ...
 *
 * Regex metacharacters are escaped before wildcards are expanded.
 */
