  ...
  return {
    async getCachedProductIds(filterId) {
      const filterProductIdCache = await FilterProductIdCache.find({
        filterId,
      }, { projection: { productIds: 1, filterOptionValue: 1 } }).toArray();

      const allProductIds = filterProductIdCache
        .find(cache => cache.filterOptionValue === null)?.productIds || [];
      const productIdsMap = Object.fromEntries(filterProductIdCache
        .filter(cache => cache.filterOptionValue !== null).map(cache => [cache.filterOptionValue, cache.productIds]))
      return [allProductIds, productIdsMap];
    },
    async setCachedProductIds(filterId, productIds, productIdsMap) {
      const baseCacheId = await updateIfHashChanged(FilterProductIdCache, { filterId, filterOptionValue: null }, { productIds });
      const cacheIds = await Promise.all(Object.entries(productIdsMap)
        .map(([filterOptionValue, optionProductIds]) => updateIfHashChanged(FilterProductIdCache, { filterId, filterOptionValue }, { productIds: optionProductIds })
        ));
