  ...
  try {
    if (req.method === 'POST') {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        // (which close over it) act on current request-scoped data. Same principal,
        // already enforced above, so this only updates data the caller already has.
      ...
      // Refresh the session's context holder (see the POST branch) before dispatching.
      Object.assign(transports[sessionId].context, req.unchainedContext);
