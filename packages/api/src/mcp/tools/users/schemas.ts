...
export const UserProfileSchema = z.object({
  ...
  }).optional().describe('User\'s address information for billing and shipping')
});
...
export const actionValidators = {
  LIST: z.object({
    ...
    }).optional().describe('Filter by last login date range'),
  }),
  ...
  ENROLL: z.object({
    ...
    password: z.string().optional().describe('Optional password (if not provided, enrollment email sent)'),
  }),
...
export const UsersManagementSchema = {
  action: z
    .enum([
      'LIST', 'COUNT', 'GET', 'CREATE', 'UPDATE', 'REMOVE', 'ENROLL',
      'SET_ROLES', 'SET_TAGS', 'SET_PASSWORD', 'SET_USERNAME',
      'ADD_EMAIL', 'REMOVE_EMAIL', 'SEND_ENROLLMENT_EMAIL', 'SEND_VERIFICATION_EMAIL',
      'REMOVE_PRODUCT_REVIEWS', 'GET_ORDERS', 'GET_ENROLLMENTS', 'GET_QUOTATIONS',
      'GET_BOOKMARKS', 'GET_PAYMENT_CREDENTIALS', 'GET_AVATAR', 'GET_REVIEWS', 'GET_REVIEWS_COUNT'
    ])
    .describe(
  ...
  userId: z.string().optional().describe('Required for: GET, UPDATE, REMOVE, SET_ROLES, SET_TAGS, SET_PASSWORD, SET_USERNAME, GET_ORDERS, GET_ENROLLMENTS, GET_QUOTATIONS, GET_BOOKMARKS, GET_PAYMENT_CREDENTIALS, GET_AVATAR, GET_REVIEWS, GET_REVIEWS_COUNT, REMOVE_PRODUCT_REVIEWS. Optional for: ADD_EMAIL, REMOVE_EMAIL (defaults to current user if not provided)'),

  username: z.string().optional().describe('Used for: CREATE (optional username for new user), SET_USERNAME (new username to assign)'),
  email: z.string().optional().describe('Used for: CREATE (optional email for new user), ENROLL (required email for enrollment), ADD_EMAIL/REMOVE_EMAIL (email address to add/remove), SEND_ENROLLMENT_EMAIL/SEND_VERIFICATION_EMAIL (target email address)'),
  password: z.string().optional().describe('Used for: CREATE (optional password for new user), ENROLL (optional password - if not provided, enrollment email is sent)'),
  ...
  roles: z.array(z.string()).optional().describe('Used for: SET_ROLES (array of role names to assign to user, e.g. ["admin", "editor"])'),
  tags: z.array(z.string()).optional().describe('Used for: SET_TAGS (array of tag names to assign to user for categorization/filtering)'),
  removeUserReviews: z.boolean().optional().describe('Used for: REMOVE (if true, also delete all product reviews authored by the user when removing the user account)'),
  ...PaginationSchema,
  ...SortingSchema,
  ...SearchSchema,
  ...DateRangeSchema,
  includeGuests: z.boolean().optional().describe('Used for: LIST, COUNT (if true, include guest/anonymous users in results, default: false)'),
  emailVerified: z.boolean().optional().describe('Used for: LIST, COUNT (filter users by email verification status - true: only verified emails, false: only unverified emails, undefined: all users)'),
  lastLogin: z.object({
  ...
  }).optional().describe('Used for: LIST, COUNT (filter users by their last login date range)'),

  includeCarts: z.boolean().optional().describe('Used for: GET_ORDERS (if true, include cart/open orders in results along with completed orders, default: false)'),
  status: z.array(z.string()).optional().describe('Used for: GET_ORDERS (filter by order status like ["PENDING", "CONFIRMED"]), GET_ENROLLMENTS (filter by enrollment status like ["ACTIVE", "PAUSED"])'),
};
