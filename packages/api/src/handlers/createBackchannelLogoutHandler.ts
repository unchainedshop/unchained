...
interface BackchannelLogoutContext {
  unchainedAPI: UnchainedCore;
  providers: OIDCProviderConfig[];
}

/**
 * Creates a WHATWG-compatible back-channel logout handler
 * Works with both Express and Fastify via @whatwg-node/server
 *
...
export function createBackchannelLogoutHandler(
  ...
  return createServerAdapter<BackchannelLogoutContext>(async (request: Request) => {
    ...
    try {
      ...
      const user = await unchainedAPI.modules.users.findUserById(sub);
      ...
      await unchainedAPI.modules.users.updateOidcLogoutAt(user._id, new Date());
