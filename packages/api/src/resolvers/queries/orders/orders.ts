...
export default async function orders(
  ...
  const { limit, offset, filter, ...restParams } = params;
  ...
  if (filter?.paymentProviderTypes?.length) {
    promises.push(
      modules.payment.paymentProviders
        .findProviders({ type: { $in: filter.paymentProviderTypes as PaymentProviderType[] } })
        .then((providers) => (paymentProviderIds = providers.map((p) => p._id))),
    );
  }

  if (filter?.deliveryProviderTypes?.length) {
    promises.push(
      modules.delivery
        .findProviders({ type: { $in: filter.deliveryProviderTypes as DeliveryProviderType[] } })
