...
// Configure delivery before subscribing (the platform normally configures an adapter).
const emitter = new EventEmitter();
setEmitAdapter({
  publish: (name, data) => { emitter.emit(name, data); },
  subscribe: (name, callback) => { emitter.on(name, callback); },
});
