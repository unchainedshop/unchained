...
export default class WriteStream {
  constructor(path, maxLength, file, permissions) {
    ...
    if (
      fdCache[this.path] &&
      !fdCache[this.path].ended &&
      !fdCache[this.path].aborted
    ) {
        ...
        bound(() => {
          if (efError) {
            ...
            throw new Meteor.Error(
              500,
              `[FilesCollection] [writeStream] [ensureFile] [Error:] ${efError}`
            );
          } else {
            fs.open(this.path, 'r+', this.permissions, (oError, fd) => {
              bound(() => {
                if (oError) {
                  ...
                  throw new Meteor.Error(
                    500,
                    `[FilesCollection] [writeStream] [ensureFile] [open] [Error:] ${oError}`
                  );
                } else {
                  this.fd = fd;
    ...
    if (!this.aborted && !this.ended) {
      if (this.fd) {
        fs.write(
          ...
          0,
          chunk.length,
          (num - 1) * this.file.chunkSize,
          (error, written, buffer) => {
            bound(() => {
              callback && callback(error, written, buffer);
              if (error) {
                console.warn(
                  '[FilesCollection] [writeStream] [write] [Error:]',
                  error
                );
                this.abort();
              } else {
