...
export default class FilesCollection extends FilesCollectionCore {
  constructor(config) {
    ...
    if (!helpers.isNumber(this.permissions)) {
      this.permissions = 0o644;
    }

    if (!helpers.isNumber(this.parentDirPermissions)) {
      this.parentDirPermissions = 0o755;
    }
    ...
    if (
      !helpers.isBoolean(this.allowedOrigins) ||
    ...
    if (!this.disableUpload) {
      ...
      _methods[this._methodNames._Remove] = function (selector) {
        ...
        if (self.allowClientCode) {
          ...
          throw new Meteor.Error(
            401,
            '[FilesCollection] [remove] Run code from client is not allowed!'
          );
        }
      };
    ...
    fs.ensureFile(opts.path, (efError) => {
      bound(() => {
        if (efError) {
          ...
          this._debug(
            `[FilesCollection] [write] [ensureFile] [Error:] ${fileName} -> ${opts.path}`,
            efError
          );
        } else {
    ...
    fs.ensureFile(opts.path, (efError) => {
      bound(() => {
        if (efError) {
          callback && callback(efError);
          this._debug(
            `[FilesCollection] [load] [ensureFile] [Error:] ${fileName} -> ${opts.path}`,
            efError
          );
        } else {
          request(
            ...
            (reqError, response) =>
              bound(() => {
                if (reqError) {
                  ...
                  this._debug(
                    `[FilesCollection] [load] [request.get(${url})] Error:`,
                    reqError
                  );
                } else {
    ...
    const respond = (stream, code) => {
      ...
      const closeStreamCb = (closeError) => {
        if (!closeError) {
          ...
          this._debug(
            `[FilesCollection] [serve(${vRef.path}, ${version})] [respond] [closeStreamCb] Error:`,
            closeError
          );
        }
      };
