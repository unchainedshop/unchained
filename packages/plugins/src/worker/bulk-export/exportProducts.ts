...
const buildProductHeaders = (locales: string[]) => [
    ...
    ...locales.flatMap((l) =>
        PRODUCT_CSV_SCHEMA.textFields.map((f) => `texts.${l}.${f}`),
    ),
    'supply.weightInGram',
...
const buildProductRow = (product: any, locales: string[]) => {
    ...
    locales.forEach((locale) => {
        ...
        PRODUCT_CSV_SCHEMA.textFields.forEach((f) => {
            row[`texts.${locale}.${f}`] = Array.isArray(text[f])
                ? text[f].join(';')
                : text[f] ?? '';
        });
    });
...
const exportProducts = async (params, unchainedAPI: UnchainedCore) => {
    ...
    const products =
        await unchainedAPI.modules.products.findProducts({
            includeDrafts,
            queryString,
            tags,
        });
    ...
    for await (const p of products) {
        const [texts, variations] = await Promise.all([
            ...
            unchainedAPI.modules.products.variations.findProductVariations(
                { productId: p._id },
            ),
        ]);
        ...
        const normalizedVariations = await Promise.all(
            variations.map(async (v: any) => {
                ...
                return {
                    ...v,
                    texts: Object.fromEntries(
                        variationTexts.map((t: any) => [t.locale, t]),
                    ),
    ...
    const locales = Array.from(
        new Set(
            Object.values(normalized.products).flatMap(
                (p: any) => p.texts?.map((t: any) => t.locale) ?? [],
            ),
        ),
    );
    ...
    for (const pid in normalized.products) {
        productRows.push(
            buildProductRow(normalized.products[pid], locales),
        );
        priceRows.push(
            ...buildPriceRows(pid, normalized.prices[pid]),
        );
        bundleRows.push(
            ...buildBundleRows(pid, normalized.bundles[pid]),
        );

        const { variationRows: vr, optionRows: or } =
            buildVariationRows(
                pid,
    ...
    return {
        products: toCSV(
            buildProductHeaders(locales),
            productRows,
        ),
        prices: toCSV(buildPriceHeaders(), priceRows),
        bundles: toCSV(buildBundleHeaders(), bundleRows),
        variations: toCSV(
            buildVariationHeaders(locales),
            variationRows,
        ),
        variationOptions: toCSV(
