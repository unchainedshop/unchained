...
const testPlugin = {
  key: 'shop.unchained.tests.worker-allocation',
  label: 'Worker allocation tests',
  version: '1.0.0',
...
const registerAdapter = (
  ...
  testPlugin.adapters.push(adapter);
  t.after(() => {
    testPlugin.adapters.splice(testPlugin.adapters.indexOf(adapter), 1);
  });
};

describe('WorkerDirector Mongo allocation', { timeout: 15000 }, () => {
  before(async () => {
    // Both directors read the registry; leave unrelated plugins untouched.
