...
import { flattenSelector } from './utils/flattenSelector';
...
export var configureCollection2 = function (db) {
    /**
     ...
     */
    db.Collection.prototype.attachSchema = function c2AttachSchema(ss, options) {
        options = options || {};
        ...
        function attachTo(obj) {
            ...
            if (typeof options.selector === 'object') {
                ...
                for (schemaIndex = obj._c2._simpleSchemas.length - 1; 0 < schemaIndex; schemaIndex--) {
                    ...
                    if (schema && isEqual(schema.selector, options.selector))
                        break;
                }
                if (schemaIndex <= 0) {
     ...
     */
    db.Collection.prototype.simpleSchema = function (doc, options, query) {
        if (!this._c2)
            return null;
        ...
        if (schemas && schemas.length > 0) {
            var schema = void 0, selector = void 0, target = void 0;
    ...
    ['insert', 'update'].forEach(function (methodName) {
        var _super = db.Collection.prototype[methodName];
        db.Collection.prototype[methodName] = function () {
            var args = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                args[_i] = arguments[_i];
            }
            var options = methodName === 'insert' ? args[1] : args[2];
...
function doValidate(collection, type, args, getAutoValues, userId, isFromTrustedCode) {
    ...
    if (picks && omits) {
        ...
        schema = schema.pick.apply(schema, picks);
    }
    else if (omits) {
        schema = schema.omit.apply(schema, omits);
    }
    ...
    var extendAutoValueContext = __assign(__assign(__assign({}, ((schema._cleanOptions || {}).extendAutoValueContext || {})), autoValueContext), options.extendAutoValueContext);
    ...
    schema.clean(doc, __assign(__assign(__assign(__assign({ mutate: true, isModifier: type !== 'insert' }, defaultCleanOptions), (schema._cleanOptions || {})), cleanOptionsForThisOperation), { extendAutoValueContext: extendAutoValueContext, // This was extended separately above
    ...
    if (options.validate === false) {
        ...
        isValid = validationContext.validate(docToValidate, {
            ...
            upsert: isUpsert,
            extendedCustomContext: __assign({ isInsert: type === 'insert', isUpdate: type === 'update' && options.upsert !== true, isUpsert: isUpsert, userId: userId, isFromTrustedCode: isFromTrustedCode, docId: docId }, (options.extendedCustomContext || {}))
        });
    }
    if (isValid) {
        // Add the ID back
        ...
        error = getErrorObject(validationContext, ((_c = (_b = (_a = Meteor.settings) === null || _a === void 0 ? void 0 : _a.packages) === null || _b === void 0 ? void 0 : _b.collection2) === null || _c === void 0 ? void 0 : _c.disableCollectionNamesInValidation)
            ? ''
...
function getErrorObject(context, appendToMessage) {
    if (appendToMessage === void 0) { appendToMessage = ''; }
    var message;
    var invalidKeys = typeof context.validationErrors === 'function'
        ? context.validationErrors()
        : context.invalidKeys();
    if (invalidKeys.length) {
        var firstErrorKey = invalidKeys[0].name;
        var firstErrorMessage = context.keyErrorMessage(firstErrorKey);
...
function addUniqueError(context, errorMessage) {
    ...
    var addValidationErrorsPropName = typeof context.addValidationErrors === 'function'
        ? 'addValidationErrors'
        : 'addInvalidKeys';
...
function wrapCallbackForParsingServerErrors(validationContext, cb) {
    var addValidationErrorsPropName = typeof validationContext.addValidationErrors === 'function'
        ? 'addValidationErrors'
        : 'addInvalidKeys';
...
function defineDeny(c, options) {
    if (!alreadyDefined[c._name]) {
        ...
        c.deny(__assign({ insert: function (userId, doc) {
                ...
                return false;
            }, fetch: ['_id'] }, (options.transform === true ? {} : { transform: null })));
...
export { Collection2 };
//# sourceMappingURL=configureCollection2.js.map