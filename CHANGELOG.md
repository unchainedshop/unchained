...
## Minor
- **EMAIL worker: SMTP transport fallback.** `MAIL_URL` now accepts multiple whitespace-separated SMTP connection URLs, tried in order per email until one accepts the mail. Transporters are cached per URL (so `?pool=true` connections actually pool), each failover is logged as a warning, and the work item result records the winning `transport` (credentials stripped) plus the `failedAttempts` that preceded it. A single URL behaves exactly as before.
