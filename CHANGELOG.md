...
## Patch
- User role changes and deletion now preserve a hard invariant of at least one active administrator. Concurrent admin removals are serialized across replicas, GraphQL reports a typed `LastAdminError`, and Admin UI explains that another active user must be promoted before the sole administrator can be removed or demoted.
