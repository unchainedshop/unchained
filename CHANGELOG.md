...
- **Plugin keys follow their adapter key:** four single-adapter plugins had a plugin key that differed from their adapter key and now use the adapter key: Apple IAP (`shop.unchained.apple-iap`), product catalog price (`shop.unchained.pricing.product-price`), enrollment order generator (`shop.unchained.worker-plugin.generate-enrollment-orders`) and export token (`shop.unchained.worker-plugin.export-token`). Adapter keys, which are stored on providers, are unchanged. Plugin keys are only used to dedupe registrations and in logs; a duplicate registration now also logs the label of the plugin that holds the key.
- **Package cleanup:** dropped the unused `multer` / `@fastify/multipart` peers of `@unchainedshop/api` (and from the examples) and the unused `express` / `fastify` peers of `@unchainedshop/plugins`. Removed the v4-style `{ name: { configure } }` default exports from the plugin `module.ts` files and the empty legacy default export of `presets/crypto`. The Swiss/EU/UK/US tax helpers are importable via `@unchainedshop/plugins/pricing/tax/<file>`.
- **Examples declare what they import:** the examples import `@unchainedshop/api` via its public entry points (`@unchainedshop/api/fastify`, types from `@unchainedshop/api`) instead of `@unchainedshop/api/lib/**`, declare every `@unchainedshop/*` package they import, and no longer list the pre-JWT session packages (`@fastify/session`, `express-session`, `passport`).
...
- **Admin UI plugin examples build:** the bookmark-manager plugin of the kitchensink examples imports `useProducts` from `@unchainedshop/admin-ui/modules/product` (provided by the Admin UI at runtime) instead of the unbuilt `@unchainedshop/client`, and `build:plugin` installs the plugin's dependencies first, so `npm run dev` works on a fresh checkout.
- **`@unchainedshop/client` builds again:** TypeScript 6 rejected its `moduleResolution: node`, which hid two imports through the Admin UI-only `@/` alias. `useRecentExports` accepts an optional `formatDateTime` instead of importing Admin UI internals.
...
- **Breaking (Stripe plugin):** Stripe API pinned to `2026-03-25.dahlia`; peer dependency widened to `stripe >= 19 < 23`. Upgrade `stripe` to v22 if you depend on this plugin.
...
- `Web3Address.nonce` GraphQL scalar changed from `Int` to `String` to match the actual hex/base64 nonce values used in SIWE-style flows.
