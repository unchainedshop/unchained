...
test('admin: update order delivery shipping when provider type is SHIPPING', async () => {
  const { data: { updateOrderDeliveryShipping } = {} } = await graphqlFetchAsAdmin({
    query: /* GraphQL */ `
      mutation updateOrderDeliveryShipping($orderDeliveryId: ID!, $address: AddressInput, $meta: JSON) {
        updateOrderDeliveryShipping(orderDeliveryId: $orderDeliveryId, address: $address, meta: $meta) {
          _id
          provider {
            _id
            type
