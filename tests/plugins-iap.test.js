...
describe('Plugins: Apple IAP Payments', () => {
  ...
  describe('Mutation.registerPaymentCredentials (Apple IAP)', () => {
    ...
    it('checking out again with the same transaction should fail', async () => {
      const {
        errors,
        data: { addCartProduct, updateCart, emptyCart } = {},
      } = await graphqlFetch({
        query: /* GraphQL */ `
          mutation checkout(
            $paymentContext: JSON
      ...
      expect(emptyCart).toMatchObject(expect.anything());
      expect(addCartProduct).toMatchObject(expect.anything());
      expect(updateCart).toMatchObject(expect.anything());
    });
  });
  //
  // describe('Mutation.updateOrderPaymentGeneric (Apple IAP)', () => {
  //   it('stores the transactionIdentifier on the order payment', async () => {
  //     const transactionIdentifier = '1000000056';
  //
  ...
  //       paymentProvider: { _id: 'iap-payment-provider' },
  //       user: { _id: 'user' },
  //     });
  //
  ...
  //             _id
  //           }
  //           updateCart(paymentProviderId: "iap-payment-provider") {
  //             _id
  //             status
  //           }
  //           checkoutCart(paymentContext: $paymentContext) {
