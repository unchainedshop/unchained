...
import { DocumentDirector, DocumentAdapter } from 'meteor/unchained:core-documents';
...
class SmallinvoiceAPI {
  ...
  constructor({
    ...
    const { data: { items, error } } = this.get('/client/list');
  ...
  upsertClient({
    userId, firstName, lastName, company, emailAddress,
    telNumber, addressLine, postalCode, city, countryCode,
  }) {
    const mappedBody = {
      data: {
        type: 1,
    ...
    if (clientId) {
      const { data: editResult } = this.post(`/client/edit/id/${clientId}`, mappedBody);
  ...
  addConfirmation({
    clientId, currency, title, date, positions, discounts, number,
  }) {
    const { data: { error, id } } = this.post('/confirmation/add', {
      data: {
        number,
        client_id: clientId,
        currency,
  ...
  addReceipt({
    clientId, currency, title, date, positions, discounts, number, introduction,
  }) {
    const { data: { error, id } } = this.post('/receipt/add', {
      data: {
        introduction,
        number,
        client_id: clientId,
  ...
  addInvoice({
    ...
    const { data: { error, id } } = this.post('/invoice/add', {
      data: {
        introduction,
        number,
        client_id: clientId,
