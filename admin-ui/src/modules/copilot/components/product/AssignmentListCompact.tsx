    ...
    <div className="space-y-4">
      {assignments.map(({ assignment }, idx) => {
        ...
        return (
          <div
            key={idx}
            className="flex gap-4 items-center border rounded-xl p-3 shadow-sm bg-surface-input"
          >
            <ImageWithFallback
              src={image}
              alt="Product image"
            ...
            <div className="flex-1 space-y-1">
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                {Object.entries(assignment.vector).map(([key, value]: any) => (
                  <span
                    key={key}
                    className="bg-surface-raised text-slate-800 dark:text-white px-2 py-0.5 rounded"
                  >
                    {key}: {value}
                  </span>
                ))}
            ...
            {price && (
              <div className="text-right text-sm font-semibold text-text-primary min-w-[80px]">
                {price}
              </div>
            )}
            {assignedProduct && (
              <div className="ml-4">
                <Link
                  ...
                  className="text-sm px-3 py-1 rounded-md border border-border-default hover:bg-surface-raised text-slate-700 dark:text-white"
                >
