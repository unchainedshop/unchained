...
const setupPluginRuntime = () => {
  ...
  const hostDependencies: Record<string, any> = {
    react: React,
  ...
  window.__UNCHAINED_PLUGIN_DEPS__ = hostDependencies;

  if (process.env.NODE_ENV !== 'production') {
    const missing = Object.keys(SHARED_DEP_SHIMS).filter(
      (key) => !(key in hostDependencies),
    );
    const unexpected = Object.keys(hostDependencies).filter(
      (key) => !(key in SHARED_DEP_SHIMS),
    );
    if (missing.length > 0 || unexpected.length > 0) {
      const details = [
        missing.length > 0
          ? `missing host dependencies: ${missing.join(', ')}`
          : '',
        unexpected.length > 0
          ? `host dependencies without shims: ${unexpected.join(', ')}`
          : '',
      ]
        .filter(Boolean)
        .join('; ');
      console.warn(
        `admin-ui plugin runtime dependency map is out of sync: ${details}`,
      );
    }
  }
};
